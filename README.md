...
This is the ParCa analogue of [v2ecoli](https://github.com/vivarium-collective/v2ecoli),
which did the same decomposition for the simulation processes.

## Reports

- **[Comparison Report](https://vivarium-collective.github.io/v2parca/comparison_report.html)** —
  Per-step side-by-side diff between v2parca's nine-Step composite and the original
  monolithic `fitSimData_1()` in vivarium-ecoli. Each step section shows runtime
  (v2parca vs vEcoli), declared data flow (input/output ports resolved to absolute
  store paths), and state comparison (scalar fields, array distributions with KS
  p-values, per-condition `cell_specs`). Generated by
  `python scripts/compare_parca.py` against intermediates dumped by vEcoli's
  `runscripts/parca.py --save-intermediates`.

- **[Composition Diagram](https://vivarium-collective.github.io/v2parca/network.html)** —
  Interactive Cytoscape.js network of the 9-Step ParCa pipeline and the 34
  stores it wires through. Steps colored by phase (initialize, basal fitting,
  per-condition fitting, promoter binding, set conditions, final adjustments).
  Click any node to inspect its port schemas, resolved store paths, docstring,
  and class; switch between bipartite / dagre / fcose / grid layouts.
  Generated by `python scripts/network_report.py` straight from each step's
  static `INPUT_PORTS` / `OUTPUT_PORTS` manifests — no composite instantiation
  required.
